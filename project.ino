...
const byte delayval = 100; // timing delay in milliseconds
...
byte redColor = 0;
byte greenColor = 0; byte blueColor = 0;
void irIsr() {
  ir.tick();
}
void setup() {
...
bool is_error(int peak, int ind) {
  return abs(a[i] - peak) >= 100;
}
bool checkN(int peak, int ind) {
  int right = a[i];
